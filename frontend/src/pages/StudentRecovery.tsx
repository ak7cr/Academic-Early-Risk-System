...
export function StudentRecovery() {
  ...
  const riskColor = (level: string) =>
    level === "high" ? "border-red-200 bg-red-50" : level === "medium" ? "border-yellow-200 bg-yellow-50" : "border-green-200 bg-green-50";
            ...
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              <div className={`rounded-2xl p-6 border ${riskColor(risk.risk_level)}`}>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div><span className="text-gray-500">Completion</span><p className="font-bold">{risk.completion_rate}%</p></div>
                  <div><span className="text-gray-500">Overdue</span><p className="font-bold text-red-600">{risk.overdue_tasks}</p></div>
                  <div><span className="text-gray-500">Workload</span><p className="font-bold">{risk.workload_score.toFixed(1)}</p></div>
                </div>
              </div>
              ...
              {risk.explanation.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <h3 className="text-lg font-bold text-gray-900 mb-3">Why This Risk Level?</h3>
              ...
              {risk.recommendations.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-gray-900">Recovery Steps</h3>
                  </div>
