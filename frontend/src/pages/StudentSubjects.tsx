...
import { LayoutDashboard, ListTodo, BookOpen, Calculator, HeartPulse, FileText, Loader2 } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto p-8">
          {loading ? (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">No Subjects</h2>
              <p className="text-gray-600">You don't have any enrolled subjects.</p>
            </div>
          ) : (
                ...
                return (
                  <div key={s.id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-bold text-gray-900">{s.code}</h3>
                        <p className="text-sm text-gray-600">{s.name}</p>
                        {s.semester && <p className="text-xs text-gray-400 mt-1">{s.semester}</p>}
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Tasks: {s.completed_tasks}/{s.total_tasks}</span>
